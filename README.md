...
本擴充功能**不收集、不傳送任何個人資料**至任何伺服器。

所有設定（自選股清單、別名、輪播秒數）僅儲存於使用者本機的 `chrome.storage.local`，不會離開使用者裝置。

網路請求僅向以下公開 API 發出，用途限於取得股票報價，不包含任何個人識別資訊：
- 台灣證券交易所 TWSE MIS API（台股即時資料）
- Yahoo Finance API（美股報價）

Content script 注入所有網頁，用途僅限於修改分頁標題與 favicon，**不讀取、不傳送網頁內容**。
